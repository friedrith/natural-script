...
describe('parse', function() {
  ...
  describe('3 words', function() {
    it('should return true', function() {
      expect(ns.parse('testaaa testbbb testccc', 'testaaa testbbb testccc')).to
        .be.true
      expect(ns.parse('testaaa,testbbb', 'testaaa testbbb')).to.be.true
      expect(ns.parse('testaaa:testbbb', 'testaaa testbbb')).to.be.true
      expect(ns.parse('testaaa-testbbb', 'testaaa testbbb')).to.be.true
    ...
    it('should return false', function() {
      expect(ns.parse('testaaaa testbbbb testccc', 'testbbbb testaaa testccc'))
        .to.be.false
    })
  })
            ...
            integers.forEach(function(integer) {
              urls.forEach(function(url) {
                var s =
                  ...
                  url
                var ref =
                  'testaaa testbbbb {{date}} {{capital_city}} {{color}} {{occurrence}} {{integer}} {{url}}'
                it(
                  '"' + s + '" -> "' + ref + '" should return true',
                  function() {
                    // console.log(ns.parse(s, ref));
                    expect(ns.parse(s, ref)).to.be.true
                  }
